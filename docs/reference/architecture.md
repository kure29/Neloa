...
本页是架构与安全边界的中文译文。**[英文原文](/en/reference/architecture)是唯一真源**，两者出现分歧时以英文原文为准。
...
支持 `streaming-file-hash` 能力的双方只在文件邀请中提供脱敏后的基本名称和字节大小。发送方在读取、加密和发送文件的同时计算 SHA-256，并在经过认证的完成消息中提交摘要；接收方同步计算摘要并进行比对。缺少该能力的旧客户端仍使用发送前预哈希。接收方只写入该传输专用的临时文件，拒绝非法偏移或多余字节，校验最终摘要，刷写到磁盘，并在不替换已有文件的前提下发布。取消和失败只会删除该传输自己的临时文件。
...
中继服务位于 `relay/`，共享的线协议定义位于 `crates/neloa-relay-protocol`。它用一个服务器令牌为小型单用户部署做鉴权，把在线状态和隧道状态保存在内存中，并转发有界的二进制帧而不检查其中经过 Noise 加密的内容。客户端维护一条 WebSocket 连接，把中继隧道映射为有界的本地字节流，并复用既有的配对、传输、测试消息和剪贴板 Noise 协议，不引入与传输相关的额外信封。共享令牌只限定可见设备范围，设备信任仍由 Noise 握手、六位验证码和双方确认建立。
