...
中继解决一个问题：两台已配对的设备不在同一网络时，怎么让它们连上。它**不**解决配对——跨网络之前，先在局域网完成一次配对。
...
1. 先在局域网完成两台设备的配对。
2. 部署中继并准备好同一个至少 32 字符的令牌。
3. 在每台设备上打开「设置 → 连接 → 自建中继」。
4. 填入公网的 `wss://.../v1/ws` 地址和同一个令牌，打开开关后保存。

地址和启用状态保存在应用数据目录里，令牌单独保存在系统凭据库中。令牌输入框留空表示不更换已保存的值。

当一台已配对设备只能通过中继到达时，它的雷达状态会显示「已配对 · 中继」。关闭中继不会影响局域网传输。
