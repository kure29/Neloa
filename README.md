...
The `Build Installers` workflow runs on demand from the repository's **Actions** tab and whenever a `v*` tag is pushed. Manual runs ask for a test release tag, defaulting to `v0.1.5-test`. A successful run creates or updates a draft prerelease with downloadable assets:
