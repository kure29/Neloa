...
Android release signing and ABI-specific ARM64/x86_64 APK builds are configured through GitHub Actions. The iOS Xcode project, local-network declarations, and signing entitlement are generated, but an iOS binary still requires full Xcode, an Apple development team, and real-device validation. Pairing throttling, settings migration, desktop release signing, and cross-device acceptance testing remain product milestones.
...
Local build artifacts are written to `releases/`, which is intentionally excluded from Git history; distributable binaries should be attached to GitHub Releases. Android test releases provide separate signed ARM64 and x86_64 Release APKs. The iOS project is at `src-tauri/gen/apple/neloa.xcodeproj`; an IPA cannot be produced without full Xcode and Apple signing. See `MOBILE_BUILD.md` for signing setup, installation, rebuild commands, platform limitations, and the real-device acceptance checklist.
...
The `Build Installers` workflow runs on demand from the repository's **Actions** tab and whenever a `v*` tag is pushed. Manual runs ask for a test release tag, defaulting to `v0.1.6-test`. A successful run creates or updates a draft prerelease with downloadable assets:
...
- separate ARM64 and x86_64 Android Release APKs signed with the repository's permanent Android identity.

Android builds require the `ANDROID_KEYSTORE_BASE64` and `ANDROID_KEYSTORE_PASSWORD` repository secrets; run `./scripts/setup-android-signing.sh` once to create and upload them. Draft releases are used instead of Actions artifacts so packages do not consume the account's limited Actions/Packages artifact allowance. iOS, macOS, and Windows production signing remain excluded until their platform credentials are configured. Build artifacts belong in GitHub Releases and must not be committed to the repository.
