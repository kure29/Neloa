...
中继不会代替首次配对。先完成上面的局域网配对，再进行跨网络验证：

1. 将中继部署在有效 HTTPS 证书之后，准备公共 `wss://.../v1/ws` 地址和同一个至少 32 字符的令牌。
2. 在两端打开“设置 → 高级 → 连接与诊断 → 自建中继”，填入相同地址和令牌并保存，确认状态为“已连接”。
3. 让手机切到蜂窝网络或另一个 Wi-Fi；对应设备应重新出现，并显示“已配对 · 中继”。
4. 双向发送测试文本、小文件和较大文件，再验证前台剪贴板同步；传输中的加密、确认、校验和取消行为应与局域网一致。
